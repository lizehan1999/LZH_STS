...
def Sbert_loss(pred_t1, pred_t2, labels):
    # pred.shape: [bs dim * k]
    # labels = torch.squeeze(labels, dim=-1)
    criteria = nn.MSELoss()
    output = torch.cosine_similarity(pred_t1, pred_t2).to(torch.float32)
    loss = criteria(output, labels.view(-1).to(torch.float32)).to(torch.float32)
    return loss
...
def evaluate_test(model, dataloader, config):
    ...
    with torch.no_grad():
        for index, [t1, t2, l] in enumerate(dataloader):
            ...
            pred_t1, pred_t2 = model(input_ids_t1, attention_mask_t1, token_type_ids_t1, input_ids_t2,
                                     attention_mask_t2,
                                     token_type_ids_t2)
...
def train_eval(model, optimizer, train_dataloader, dev_dataloader, config):
    ...
    for epoch in range(config.epochs):
        ...
        for step, [t1, t2, l] in enumerate(train_dataloader):
            ...
            loss = Sbert_loss(pred_t1, pred_t2, labels)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            # scheduler.step()
